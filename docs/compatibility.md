...
|---|---:|---|
| Source catalog | 1 | Accepted until a documented compiler release removes it. |
| Source resource schema | 1 and 2 | Schema 2 is the current authoring surface; schema 1 remains frozen. |
| Message grammar / normalized AST | 2 | Every generated backend consumes the same compiler-owned AST. |
| Locale artifact | 1 and 2 | Decoders reject unsupported versions before reading messages. |
...
Use one exact package version for the seven `RunicTranslations.*` NuGet packages,
the `RunicTranslations.Tool` local tool, and

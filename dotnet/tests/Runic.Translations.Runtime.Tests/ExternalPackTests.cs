...
internal static class ExternalPackTests
{
    ...
    public static void Register(TestRunner runner)
    {
        ...
        runner.Add("external pack isolates parser bytes from verifier mutation", IntegrityMutationIsolation);
        runner.Add("external pack exposes stable failure reasons", FailureReasons);
        runner.Add("external pack exposes normalized locale-pack rejection IDs", RejectionIds);
        runner.Add("external pack matches the shared locale-pack rejection corpus", RejectionParityCorpus);
    }
    ...
    private static async Task Version2StructuralFailures()
    {
        TranslationPackContract contract = new(
            ...
            [new TranslationPackMessageContract(new TranslationKey("app", 0, "Summary"),
                [new TranslationPackArgumentContract("count", TextArgumentType.Int, TextArgumentFormat.Plain)])],
            messageGrammarVersion: 2);
        string PackMessage(string message) => "{\"artifactVersion\":2,\"messageGrammarVersion\":2,\"catalog\":\"app\",\"locale\":\"en-US\",\"contractFingerprint\":\"" + Fingerprint + "\",\"messages\":{\"Summary\":" + message + "}}";
        string inputs = "\"inputs\":{\"count\":{\"type\":\"int\",\"format\":\"plain\"}}";
        string selectors = string.Join(',', Enumerable.Range(0, 17).Select(index => "{\"name\":\"s" + index + "\",\"input\":\"count\",\"function\":\"plural\"}"));
        await AssertReason(() => Verify(PackMessage("{\"astVersion\":2," + inputs + ",\"selectors\":[" + selectors + "],\"variants\":[]}"), contract), TranslationPackFailureReason.LimitExceeded);

        const string oneVariant = "{\"matches\":{},\"nodes\":[{\"kind\":\"text\",\"value\":\"x\"}]}";
        await AssertReason(() => Verify(PackMessage("{\"astVersion\":2,\"inputs\":{},\"selectors\":[],\"variants\":[" + string.Join(',', Enumerable.Repeat(oneVariant, 257)) + "]}"), new TranslationPackContract("app", "en-US", Fingerprint, [new TranslationPackMessageContract(new TranslationKey("app", 0, "Summary"), Array.Empty<TranslationPackArgumentContract>())], messageGrammarVersion: 2)), TranslationPackFailureReason.LimitExceeded);

        string node = "{\"kind\":\"text\",\"value\":\"x\"}";
        for (int depth = 0; depth < 17; depth++) node = "{\"kind\":\"markup\",\"name\":\"m\",\"attributes\":{},\"children\":[" + node + "]}";
        await AssertReason(() => Verify(PackMessage("{\"astVersion\":2," + inputs + ",\"selectors\":[],\"variants\":[{\"matches\":{},\"nodes\":[" + node + "]}]}"), contract), TranslationPackFailureReason.LimitExceeded);
        await AssertReason(() => Verify(PackMessage("{\"astVersion\":2," + inputs + ",\"selectors\":[],\"variants\":[{\"matches\":{},\"nodes\":[],\"future\":true}]}"), contract), TranslationPackFailureReason.UnknownMember);
    }
    ...
    private static async Task RejectionIds()
    {
        TranslationPackException exception = await Assert.ThrowsAsync<TranslationPackException>(
        ...
        Assert.Equal("RTR0023/contract-fingerprint-mismatch", TranslationPackFailure.GetRejectionId(exception));
    }

    private static async Task RejectionParityCorpus()
    {
        using JsonDocument document = JsonDocument.Parse(File.ReadAllBytes(FindCorpusFile("locale-pack-v2-parity.json")));
        JsonElement limits = document.RootElement.GetProperty("limits");
        Assert.Equal(TranslationPackLimits.DefaultMaximumDocumentBytes, limits.GetProperty("maximumDocumentBytes").GetInt32(), "The corpus document limit drifted from the runtime default.");
        Assert.Equal(TranslationPackLimits.DefaultMaximumDepth, limits.GetProperty("maximumDepth").GetInt32(), "The corpus depth limit drifted from the runtime default.");
        Assert.Equal(TranslationPackLimits.DefaultMaximumMessages, limits.GetProperty("maximumMessages").GetInt32(), "The corpus message limit drifted from the runtime default.");
        Assert.Equal(TranslationPackLimits.DefaultMaximumPatternBytes, limits.GetProperty("maximumPatternBytes").GetInt32(), "The corpus pattern limit drifted from the runtime default.");
        Assert.Equal(TranslationPackLimits.DefaultMaximumArgumentsPerMessage, limits.GetProperty("maximumArgumentsPerMessage").GetInt32(), "The corpus argument limit drifted from the runtime default.");
        int executed = 0;
        foreach (JsonElement item in document.RootElement.GetProperty("rejectionParity").EnumerateArray())
        {
            string id = item.GetProperty("id").GetString()!;
            string expected = item.GetProperty("expected").GetString()!;
            byte[] bytes = Encoding.UTF8.GetBytes(BindIdentity(item.GetProperty("template").GetString()!));
            if (item.TryGetProperty("truncateFromEnd", out JsonElement truncate))
                bytes = bytes[..(bytes.Length - truncate.GetInt32())];
            if (item.TryGetProperty("padTo", out JsonElement pad))
            {
                byte[] padded = new byte[pad.GetInt32()];
                bytes.CopyTo(padded, 0);
                padded.AsSpan(bytes.Length).Fill((byte)' ');
                bytes = padded;
            }
            string locale = item.TryGetProperty("expectedLocale", out JsonElement expectedLocale) ? BindIdentity(expectedLocale.GetString()!) : "en-US";
            var contract = new TranslationPackContract("parity", locale, Fingerprint,
                [new TranslationPackMessageContract(new TranslationKey("parity", 0, "Parity.Message"),
                    [new TranslationPackArgumentContract("name", TextArgumentType.String, TextArgumentFormat.None)])],
                messageGrammarVersion: 2);
            bool rejectingVerifier = item.TryGetProperty("verifier", out JsonElement verifier) && verifier.GetString() == "reject";
            try
            {
                await TranslationPackLoader.VerifyAsync(new ExternalTranslationPack(bytes), contract,
                    integrityVerifier: rejectingVerifier ? static (_, _) => ValueTask.FromResult(false) : null);
                Assert.True(expected == "accepted", "Parity case " + id + " was accepted; expected " + expected + ".");
            }
            catch (TranslationPackException exception)
            {
                Assert.False(expected == "accepted", "Parity case " + id + " was rejected: " + exception.Message);
                Assert.Equal(expected, TranslationPackFailure.GetRejectionId(exception), "Parity case " + id + " surfaced a divergent rejection reason.");
            }
            executed++;
        }
        Assert.True(executed >= 12, "The locale-pack rejection parity corpus lost cases.");
    }

    private static string BindIdentity(string template)
    {
        foreach ((string token, string value) in IdentityBindings) template = template.Replace(token, value, StringComparison.Ordinal);
        return template;
    }

    private static readonly (string Token, string Value)[] IdentityBindings =
[
        ("%VERSION%", "2"),
        ("%GRAMMAR%", "2"),
        ("%BAD_VERSION%", "3"),
        ...
        ("%OTHER_LOCALE%", "de-DE"),
        ("%OTHER_FINGERPRINT%", "sha256:" + new string('f', 64)),
];
    ...
    private static string FindCorpusFile(string name)
    {
        ...
        throw new DirectoryNotFoundException("Could not locate the locale-pack parity corpus file " + name + ".");
    }
    ...
    private static async Task IntegrityMutationIsolation()
    {
        VerifiedExternalTranslationPack verified = await TranslationPackLoader.VerifyAsync(
            Pack(ValidJson()), CreateContract(), integrityVerifier: static (content, token) =>
            {
                if (!MemoryMarshal.TryGetArray(content, out ArraySegment<byte> image) || image.Array is null)
                    throw new InvalidOperationException("The verifier did not receive an owned byte image.");
                image.Array[image.Offset] = (byte)'!';
                return ValueTask.FromResult(true);
            });
        Assert.Equal(2, verified.Messages.Count, "Verifier mutation altered the parser image.");
    }
    ...
    private static async Task AssertReason(Func<Task> action, TranslationPackFailureReason expected)
    {
        ...
        Assert.Equal("RTR0023/", TranslationPackFailure.RejectionIdPrefix);
        Assert.Equal(TranslationPackFailure.DiagnosticId, TranslationPackFailure.GetDiagnosticId(exception));
        Assert.True(TranslationPackFailure.GetRejectionId(exception).StartsWith(TranslationPackFailure.RejectionIdPrefix, StringComparison.Ordinal), "The rejection ID did not use the normalized prefix.");
    }

...
namespace RunicTextResources.Generator;
...
public sealed class TextResourcesGenerator : IIncrementalGenerator
{
    ...
    private static RuntimeAbiState InspectRuntimeAbi(Compilation compilation)
    {
        foreach (MetadataReference reference in compilation.References)
        {
            if (!(compilation.GetAssemblyOrModuleSymbol(reference) is IAssemblySymbol assembly) ||
                !string.Equals(assembly.Identity.Name, "RunicTextResources", StringComparison.Ordinal))
                continue;

            INamespaceSymbol? currentNamespace = NamespaceMember(assembly.GlobalNamespace, "RunicTextResources");
    ...
    private static Diagnostic CreateAbiDiagnostic(RuntimeAbiState state)
    {
        string message = state.IsMissing
            ? "Referenced RunicTextResources runtime ABI is missing; generated code requires ABI version 1."
            : "Referenced RunicTextResources runtime ABI version " + state.Version + " is incompatible with generated ABI version 1.";

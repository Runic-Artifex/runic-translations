...
namespace RunicTranslations.Generator;
...
public sealed class TranslationsGenerator : IIncrementalGenerator
{
    ...
    private static RuntimeAbiState InspectRuntimeAbi(Compilation compilation)
    {
        foreach (MetadataReference reference in compilation.References)
        {
            if (!(compilation.GetAssemblyOrModuleSymbol(reference) is IAssemblySymbol assembly) ||
                !string.Equals(assembly.Identity.Name, "RunicTranslations", StringComparison.Ordinal))
                continue;

            INamespaceSymbol? currentNamespace = NamespaceMember(assembly.GlobalNamespace, "RunicTranslations");
    ...
    private static Diagnostic CreateAbiDiagnostic(RuntimeAbiState state)
    {
        string message = state.IsMissing
            ? "Referenced RunicTranslations runtime ABI is missing; generated code requires ABI version 1."
            : "Referenced RunicTranslations runtime ABI version " + state.Version + " is incompatible with generated ABI version 1.";

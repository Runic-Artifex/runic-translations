...
internal static class TranslationPackV2Loader
{
    ...
    {
        if (value.ValueKind != JsonValueKind.Array) throw Error("A message selector list is invalid.");
        if (value.GetArrayLength() > 16) throw Limit("A message exceeds the normalized selector limit.");

...
The build task and `dotnet-textresources` tool are adapters over the same pure
compiler and renderers. They MUST report the same `WUTTEXT` diagnostic ID,
...
- `1`: compiler errors, `WUTTEXT0020`, or a `verify` difference;
...
rejected with `WUTTEXT0020`. Containment is rechecked immediately before each
replacement.

...
`WebUIToolkit.TextResources`. The standalone planning document remains the source
...
Wave A reserves `WUTTEXT0001` through `WUTTEXT0022` and `WUTTEXT0099` under ADR
...
`WUTTEXT0020` is reserved but not emitted by the TR0–TR1 compiler kernel because
output paths do not enter the pure compilation API. It becomes executable in the
later build/CLI surface. `WUTTEXT0023`, `WUTTEXT0024`, and `WUTTEXT0099` belong to

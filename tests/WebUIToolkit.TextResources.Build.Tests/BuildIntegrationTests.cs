...
internal static class BuildIntegrationTests
{
    ...
    private static void TargetDeclaresInputsAndOutputs()
    {
        string targetsPath = RepositoryPaths.Resolve(
            ...
            "build",
            "WebUIToolkit.TextResources.Build.targets");
        XDocument document = XDocument.Load(targetsPath, LoadOptions.PreserveWhitespace);
        XElement target = document
            .Descendants("Target")
            .Single(element => string.Equals((string?)element.Attribute("Name"), "_WebUIToolkitGenerateTextResourceArtifactsCore", StringComparison.Ordinal));
    ...
    private static void OutputContainmentIsEnforced()
    {
        ...
        Assert.Contains("WUTTEXT0020", result.Combined);
        ...
        string? toolCommand = null)
    {
        ...
        string fixture = RepositoryPaths.Resolve("spec", "text-resources", "corpus", "valid", "minimal");
        ...
        string props = XmlPath(RepositoryPaths.Resolve("src", "WebUIToolkit.TextResources.Build", "build", "WebUIToolkit.TextResources.Build.props"));
        string targets = XmlPath(RepositoryPaths.Resolve("src", "WebUIToolkit.TextResources.Build", "build", "WebUIToolkit.TextResources.Build.targets"));
            ...
            <Project Sdk="Microsoft.NET.Sdk">
              ...
              <Target Name="DumpTextResourceItems">
                <WriteLinesToFile File="dump.txt"
                                  Overwrite="true"
                                  Lines="PropsImported=$(WebUIToolkitTextResourcesBuildPropsImported);TargetsImported=$(WebUIToolkitTextResourcesBuildTargetsImported);@(AdditionalFiles->'%(Filename)|%(WebUIToolkitTextResourceKind)')" />
              </Target>

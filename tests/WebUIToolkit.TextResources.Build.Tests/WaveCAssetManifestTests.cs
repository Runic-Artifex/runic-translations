...
internal static class WaveCAssetManifestTests
{
    ...
    {
        string corpusRoot = RepositoryPaths.Resolve("spec", "text-resources", "corpus", "wave-c");
        ...
        Assert.Equal("WebUIToolkit.TextResources.WaveC.Conformance", index.RootElement.GetProperty("identity").GetString());
        ...
        Assert.Equal("WebUIToolkit.TextResources.WaveC.AssetManifest", root.GetProperty("identity").GetString());
    ...
    private static void HostileInputsAreRejectedOrContained()
    {
        ...
        using (TemporaryDirectory temporary = new())
        {
            ...
            Assert.Contains("WUTTEXT0001", result.StandardError);
        ...
        using (TemporaryDirectory temporary = new())
        {
            ...
            ProcessResult result = TestFixture.RunTool(temporary, "generate", "--catalog", "manifest.json", "--documents", "en.json", "--output", "generated", "--emit-json");
            Assert.Equal(1, result.ExitCode, result.Combined);
            Assert.Contains("WUTTEXT0006", result.StandardError);

...
internal static class CorpusTests
{
    ...
    private static void CorpusContract()
    {
        ...
        Assert.Equal("WebUIToolkit.TextResources.Conformance", Index.Identity);
        ...
        AssertSequence(["WUTTEXT0020"], Index.ExcludedDiagnostics);
    ...
    private static void AssertStrictJsonFailure(string path, byte[] bytes, int line, int column)
    {
        ...
        Assert.Equal("WUTTEXT0001", diagnostic.Id);
        ...
        {
            Assert.True(item.Id != "WUTTEXT0099", path + " was incorrectly reported as an internal compiler failure.");
        }
    }
    ...
    private static CompilerModel.TextResourceSource ReadCorpusSource(string path)
    {
        string absolutePath = RepositoryPaths.Resolve(
            "spec", "text-resources", "corpus", path.Replace('/', Path.DirectorySeparatorChar));
    ...
    {
        string path = RepositoryPaths.Resolve("spec", "text-resources", "corpus", "index.json");

...
internal static class ArtifactFiles
{
    ...
    internal static IReadOnlyList<ToolArtifact> Normalize(IReadOnlyList<ToolArtifact> artifacts)
    {
        ...
        for (int index = 0; index < artifacts.Count; index++)
        {
            ...
            if (!paths.Add(path))
            {
                throw new ToolOutputException($"WUTTEXT0020: generated output path collision for '{path}'.");
            }
    ...
    internal static void WriteAtomically(string outputPath, IReadOnlyList<ToolArtifact> artifacts)
    {
        ...
        if (parent is null)
        {
            throw new ToolOutputException($"WUTTEXT0020: output root '{NormalizePath(outputPath)}' is invalid.");
        }
            ...
            for (int index = 0; index < normalizedArtifacts.Count; index++)
            {
                ...
                if (Directory.Exists(destinationPaths[index]))
                {
                    throw new ToolOutputException(
                        $"WUTTEXT0020: declared output path '{artifact.RelativePath}' is an existing directory.");
                }
            }
    ...
    internal static string ResolveContainedPath(string root, string relativePath)
    {
        ...
        {
            throw new ToolOutputException($"WUTTEXT0020: output path '{relativePath}' escapes the configured output root.");
        }
    ...
    private static IEnumerable<string> EnumerateOutputFiles(string outputRoot)
    {
        ...
        while (pending.Count != 0)
        {
            ...
            for (int index = 0; index < entries.Count; index++)
            {
                ...
                {
                    throw new ToolOutputException(
                        $"WUTTEXT0020: output path '{relative}' traverses a symbolic link or reparse point.");
                }
    ...
    private static void EnsureNoReparsePoint(string path, string displayPath)
    {
        ...
        string pathRoot = Path.GetPathRoot(fullPath) ?? throw new ToolOutputException(
            $"WUTTEXT0020: output path '{displayPath}' has no filesystem root.");
        ...
        for (int index = 0; index < segments.Length; index++)
        {
            ...
            {
                throw new ToolOutputException(
                    $"WUTTEXT0020: output path '{displayPath}' traverses a symbolic link or reparse point.");
            }
        }
    }
    ...
    private static string NormalizeRelativePath(string path)
    {
        ...
        {
            throw new ToolOutputException("WUTTEXT0020: generated output path must not be empty.");
        }

        string normalized = NormalizePath(path);
        if (Path.IsPathRooted(normalized) || normalized[0] == '/')
        {
            throw new ToolOutputException($"WUTTEXT0020: generated output path '{normalized}' must be relative.");
        }
        ...
        for (int index = 0; index < segments.Length; index++)
        {
            ...
            {
                throw new ToolOutputException($"WUTTEXT0020: generated output path '{normalized}' is not canonical and contained.");
            }
    ...
    private static void ValidatePortableSegment(string segment, string relativePath)
    {
        ...
        {
            throw new ToolOutputException(
                $"WUTTEXT0020: generated output path '{relativePath}' is not portable because a component ends with a space or dot.");
        }
        ...
        for (int index = 0; index < segment.Length; index++)
        {
            ...
            {
                throw new ToolOutputException(
                    $"WUTTEXT0020: generated output path '{relativePath}' contains a non-portable character.");
            }
        }
        ...
        {
            throw new ToolOutputException(
                $"WUTTEXT0020: generated output path '{relativePath}' uses reserved device name '{baseName}'.");
        }
    }
